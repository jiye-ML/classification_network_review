...
def _get_filenames_and_classes(dataset_dir, dataset_name):
    ...
    print('processing data in [%s] :' % flower_root)
    directories = []
    class_names = []
...
def _get_dataset_filename(dataset_dir, dataset_name, split_name, shard_id):
    output_filename = '%s_%s_%05d-of-%05d.tfrecord' % (dataset_name, split_name, shard_id, _NUM_SHARDS)
            ...
            for shard_id in range(_NUM_SHARDS):
                ...
                with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
                    ...
                    for i in range(start_ndx, end_ndx):
                        print("Converting image {}/{} shard {}".format(i + 1, len(filenames), shard_id))
...
def run(dataset_dir, dataset_name='flowers'):
    ...
    if _dataset_exists(dataset_dir, dataset_name, 'train'):
        ...
        return
    _convert_dataset('train', training_filenames, class_names_to_ids, dataset_dir, dataset_name=dataset_name)
    _NUM_SHARDS = len(validation_filenames) // 1024
    _NUM_SHARDS = _NUM_SHARDS if _NUM_SHARDS else 1
    if _dataset_exists(dataset_dir, dataset_name, 'validation'):
        print('Dataset files already exist. Exiting without re-creating them.')
        return
    _convert_dataset('validation', validation_filenames, class_names_to_ids, dataset_dir, dataset_name=dataset_name)
